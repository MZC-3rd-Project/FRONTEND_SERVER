...
import { useStoresQuery } from "@/domains/client/store/query/useStoreQueries";
...
function StorePage() {
    const { data, isLoading, isError, error, refetch, isFetching } = useStoresQuery({ size: 12 });
    const stores = useMemo(() => data?.items ?? [], [data?.items]);
    const storeStatuses = useMemo(
        () => ["전체", ...new Set(stores.map((store) => store.status).filter(Boolean))],
        [stores]
    );
    ...
    return (
        <div className="grid items-start gap-4 md:grid-cols-[1fr_320px]">
            <div className="order-2 space-y-6 md:order-1">
                ...
                {isLoading ? <StoreListSkeleton /> : null}
                ...
                {!isLoading && !isError && stores.length === 0 ? (
                    <Empty className="rounded-3xl border border-border bg-card">
                        <EmptyHeader>
                            ...
                            <EmptyTitle>노출 중인 스토어가 없습니다</EmptyTitle>
                            <EmptyDescription>
            ...
            <aside className="order-1 md:order-2 md:sticky md:top-24 md:self-start">
                <StickySearchPanel
                    scope="store"
                    title="스토어 검색"
                    description="스토어명과 상태 조건으로 검색 결과 페이지로 이동합니다."
